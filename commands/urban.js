...
exports.run = (client, message, args, level) => { // eslint-disable-line no-unused-vars
	var term = args.join(' ');
	if (!message.channel.nsfw) return message.channel.send("Dude to the possible chance of NSFW content, please use this in an nsfw channel");
	...
	snekfetch.get(`https://api.urbandictionary.com/v0/define?term=${encodeURIComponent(term)}`).then((data) => {
		...
		var defEmbed = new Discord.MessageEmbed()
