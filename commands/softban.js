...
exports.run = (client, message, args) => {
  ...
  if (message.mentions.users.first().id === "523579776749928449") return message.reply("You can't ban my Developer:wink:");
  if (reason.length < 1) reason = 'No reason supplied.';
  if (!message.guild.member(user).bannable) return message.reply(`:redTick: I cannot ban that member`);
//  if (!logchannel) return message.reply('I cannot find a channel named #logs');
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setFooter(`© AngelDust by ${customisation.ownername}`);
