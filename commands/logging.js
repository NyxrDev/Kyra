...
`)
              ...
              if (option.match("set")) {
            var channel = JSON.parse(fs.readFileSync("./data/logging.json", "utf8"))
            ...
            if (args[0]) {
              ...
              fs.writeFile("./data/logging.json", JSON.stringify(channel, null, 2), (err) => {
            ...
            if (option.match("on")) {
            var welcomesetting = JSON.parse(fs.readFileSync("./data/logonoff.json", "utf8"));
            welcomesetting[message.guild.id] = {
                checker: 1
                };
                  fs.writeFile("./data/logonoff.json", JSON.stringify(welcomesetting, null, 2), (err) => {
                    console.error(err)
                 })
